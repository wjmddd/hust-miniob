...
Table::~Table()
{
  ...
  // 这玩意似乎没什么用，buffer pool在前面都 delete 完了，但是 delete 之后没赋值成 null，所以不注释会有麻烦。
    ...
    span<const AttrInfoSqlNode> attributes, StorageFormat storage_format)
{
  ...
  // 设置 LOM 的路径
  lom_.set_path(table_lom_file(base_dir, name));
...
RC Table::insert_record(Trx *trx, Record &record)
{
  ...
  for (auto index : indexes_) {
    ...
    if (index->index_meta().isUnique()) {
      ...
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to create scanner");
        return rc;
      }
      vector<FieldMeta> field_metas = index->field_metas();
      string            target;
      target.assign(record.data() + field_metas[0].offset());
      for (int i = 1; i < (int)field_metas.size(); i++) {
        target.append(record.data() + field_metas[i].offset());
      }
      Record new_record;
      while (OB_SUCC(rc = scanner.next(new_record))) {
        string temp;
        temp.assign(new_record.data() + field_metas[0].offset());
        for (int i = 1; i < (int)field_metas.size(); i++) {
          target.append(new_record.data() + field_metas[i].offset());
        }
        if (temp == target) {
...
RC Table::create_index(
    Trx *trx, const vector<const FieldMeta *> &field_metas, const char *index_name, const bool isUnique)
{
  bool any_field_meta_null = field_metas.empty();
  for (auto &field_meta : field_metas) {
    if (not field_meta) {
      any_field_meta_null = true;
      break;
    }
  }
  if (common::is_blank(index_name) || any_field_meta_null) {
    LOG_INFO("Invalid input arguments, table name is %s, index_name is blank or attribute_name is blank", name());
    return RC::INVALID_ARGUMENT;
  }

  // 遍历数据,判断索引
  RecordFileScanner newScanner;
  RC                rc = get_record_scanner(newScanner, trx, ReadWriteMode::READ_ONLY);
  if (rc != RC::SUCCESS) {
  ...
  // 查找一下已经插入的记录是否满足 Unique 的条件，若不满足则不能设置成 Unique
  if (isUnique) {
    Record      new_record;
    set<string> st;
    while (OB_SUCC(rc = newScanner.next(new_record))) {
      string temp;
      temp.assign(new_record.data() + field_metas[0]->offset());
      for (int i = 1; i < (int)field_metas.size(); i++) {
        temp.append(new_record.data() + field_metas[i]->offset());
      }
      if (st.contains(temp)) {
        LOG_WARN("failed to insert record, record is not unique");
        return RC::RECORD_NOT_UNIQUE;
  ...
  rc = new_index_meta.init(index_name, field_metas, isUnique);
  if (rc != RC::SUCCESS) {
    string field_names;
...
RC Table::update_record(Record &record, const char *attr_name, Value *value)
{
  ...
  char *old_data_backup = (char *)malloc(sizeof(char) * record.len());
  memcpy(old_data_backup, record.data(), record.len());
  ...
  } else if (targetFiled->type() == AttrType::TEXTS)  // TEXT 在这里要访问 LOM 做特殊处理
  {
    ...
    lom_.update_obj(value->data(), index);          // 修改成新的信息
  } else {
  ...
  // 更新所有的索引
  update_entry_of_indexes(targetFiled, old_data_backup, record.data(), record.rid());
  free(old_data_backup);
...
RC Table::update_entry_of_indexes(
    FieldMeta *targetFiled, const char *old_record, const char *new_record, const RID &rid)
{
  RC rc = RC::SUCCESS;
  for (Index *index : indexes_) {
    if (index->index_meta().has_field(targetFiled->name())) {
      rc = index->update_entry(old_record, new_record, &rid);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to update index. table=%s, index=%s, rc=%s", name(), index->index_meta().name(), strrc(rc));
        return rc;
      }
    }
  }
...
{
  string ret;
  lom_.find_obj(ret, (uint32_t)index);
  return ret;
}
