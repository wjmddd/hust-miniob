...
public:
  ...
  const vector<AttrComparator> &attr_comparators() const { return attr_comparators_; }
...
public:
  ...
  const vector<AttrPrinter> &attr_printers() const { return attr_printers_; }
...
struct IndexFileHeader
{
  ...
  AttrType attr_types[MAX_KEY_NUM];    ///< 键值的类型
  int32_t  attr_lengths[MAX_KEY_NUM];  ///< 键值的长度

  const string to_string() const
  {
    stringstream ss;
    string       attr_type_str = "", attr_length_str = "";
    for (auto &attr_type : attr_types) {
      attr_type_str += attr_type_to_string(attr_type);
    }
    for (int len : attr_lengths) {
      attr_length_str += std::to_string(len);
    }
...
public:
   ...
   */
  RC insert_entry(const vector<IndexUserKey> &user_keys, const RID *rid);
   ...
   * @note 这里假设user_key的内存大小与attr_length 一致
   */
  RC delete_entry(const vector<IndexUserKey> &user_keys, const RID *rid);

  RC update_entry(const vector<IndexUserKey> &old_user_keys, const vector<IndexUserKey> &new_user_keys, const RID *rid);
   ...
   */
  RC get_entry(const vector<IndexUserKey> &user_keys, int key_len, list<RID> &rids);
...
private:
  common::MemPoolItem::item_unique_ptr make_key(const vector<IndexUserKey> &user_keys, const RID *rid);
...
public:
   ...
   * TODO 重构参数表示方法
   */
  RC open(const vector<IndexUserKey> &left_user_keys, bool left_inclusive, const vector<IndexUserKey> &right_user_keys, bool right_inclusive);
...
private:
   ...
   */
  RC fix_user_key(const IndexUserKey &user_key, int attr_length, bool want_greater, IndexUserKey &fixed_key, bool &should_inclusive);
