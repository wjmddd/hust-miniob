...
RC IndexMeta::init(const char *name, const vector<const FieldMeta *> &field_metas, const bool isUnique)
{
...
RC IndexMeta::init(const string &name, const vector<FieldMeta> &field_metas, const bool isUnique)
{
  if (name.empty()) {
    LOG_ERROR("Failed to init index, name is empty.");
    return RC::INVALID_ARGUMENT;
...
const vector<FieldMeta> &IndexMeta::field_metas() const { return field_metas_; }
const bool               IndexMeta::isUnique() const { return isUnique_; }
...
bool IndexMeta::has_field(const string &field_name) const
{
  for (const FieldMeta &field_meta : field_metas_) {
