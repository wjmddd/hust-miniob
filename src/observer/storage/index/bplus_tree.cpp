...
int calc_internal_page_capacity(const vector<int> &attr_lengths)
{
  int item_size = 0;
...
int calc_leaf_page_capacity(const vector<int> &attr_lengths)
{
  int item_size = 0;
  for (int attr_length : attr_lengths) {
    item_size += attr_length;
    ...
    int leaf_max_size /* = -1 */)
{
  ...
  file_header->key_length = (int32_t)(key_length + sizeof(RID));
...
MemPoolItem::item_unique_ptr BplusTreeHandler::make_key(const vector<IndexUserKey> &user_keys, const RID *rid)
{
...
RC BplusTreeHandler::insert_entry(const vector<IndexUserKey> &user_keys, const RID *rid)
{
...
RC BplusTreeHandler::get_entry(const vector<IndexUserKey> &user_keys, int key_len, list<RID> &rids)
{
...
RC BplusTreeHandler::delete_entry(const vector<IndexUserKey> &user_keys, const RID *rid)
{
  auto pkey = make_key(user_keys, rid);
...
RC BplusTreeHandler::update_entry(const vector<IndexUserKey> &old_user_keys, const vector<IndexUserKey> &new_user_keys, const RID *rid)
{
...
RC BplusTreeScanner::open(const vector<IndexUserKey> &left_user_keys, bool left_inclusive,
    const vector<IndexUserKey> &right_user_keys, bool right_inclusive)
{
  RC rc = RC::SUCCESS;
  if (inited_) {
  ...
  if ((int32_t)left_user_keys.size() > tree_handler_.file_header_.attr_num) {
    // 边界的长度超过了属性个数
  ...
  if (left_user_keys.size() == 0) {  // 没有指定左边界，返回最左边的数据
