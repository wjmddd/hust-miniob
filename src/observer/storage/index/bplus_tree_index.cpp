...
RC BplusTreeIndex::create(Table *table, const char *file_name, const IndexMeta &index_meta)
{
  ...
  vector<AttrType>   field_types;
  vector<int>        field_lengths;
  for (const auto &field_meta : field_metas()) {
    field_types.push_back(field_meta.type());
    field_lengths.push_back(field_meta.len());
  }
  RC rc = index_handler_.create(table->db()->log_handler(), bpm, file_name, field_types, field_lengths);
  if (RC::SUCCESS != rc) {
...
RC BplusTreeIndex::make_user_keys(const char *record, std::vector<IndexUserKey> &user_keys)
{
  ...
  for (const auto &field_meta : field_metas()) {
    ...
    memcpy(user_key_tmp, record + field_meta.offset(), field_meta.len());
    user_keys.push_back(IndexUserKey(user_key_tmp, key_len));
  }
  return RC::SUCCESS;
}

IndexScanner *BplusTreeIndex::create_scanner(const vector<IndexUserKey> &left_keys, bool left_inclusive, const vector<IndexUserKey> &right_keys, bool right_inclusive)
{
...
RC BplusTreeIndexScanner::open(const vector<IndexUserKey> &left_keys, bool left_inclusive,
    const vector<IndexUserKey> &right_keys, bool right_inclusive)
{
