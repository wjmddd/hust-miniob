...
bool RecordPageIterator::clean_record_page_handler_()
{
  bool suc = false;  // 表示是否执行了 clean，防止 RecordPageHandler 被 delete 两次
...
RC RecordFileScanner::close_scan()
{
  ...
  bool is_cleaned = record_page_iterator_.clean_record_page_handler_();
  if (record_page_handler_ != nullptr) {
    if (not is_cleaned) {
      record_page_handler_->cleanup();
      delete record_page_handler_;
