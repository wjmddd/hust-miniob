    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  if (0 == strcmp(field_name, "*")) {
    ...
    FieldExpr *field_expr = new FieldExpr(field);
    string     final_field_name;
    if (context_.query_tables().size() > 1)  // 当 select 涉及不止一张表时，结果的表头需要是（表名.列名）的格式
    {
      final_field_name = string(table_name) + "." + string(field_name);
    } else {
      final_field_name = string(field_name);
    }
    field_expr->set_name(final_field_name);
    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  // 聚合多参数时,这个字段将是nullptr,语法构造出来的,所以需要特别判断一下。
