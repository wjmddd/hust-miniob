...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  bool        any_attribute_name_blank = create_index.attribute_names.empty();
  for (auto &name : create_index.attribute_names) {
    if (is_blank(name.c_str())) {
      any_attribute_name_blank = true;
      break;
    }
  }
  if (any_attribute_name_blank or is_blank(table_name) or is_blank(create_index.index_name.c_str())) {
