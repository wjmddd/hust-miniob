...
//     bool null_expr_exist =
//         left->is_null() or
//         right->is_null();  // 左右值中存在 NULL，因为 NULL 此时的类型是 undefined，所以要跳过类型转换。

//     if (not null_expr_exist and left->value_type() != right->value_type()) {
  ...
  for (auto &condition : conditions) {
    ...
    switch (condition->type()) {
      case ExprType::COMPARISON: {
        ...
        auto &left      = cmp_expr_->left();
        auto &right     = cmp_expr_->right();
        bool  null_expr_exist = left->is_null() or right->is_null();  // 左右值中存在 NULL，因为 NULL 此时的类型是 undefined，所以要跳过类型转换。
        bool is_sub_query = left->type() == ExprType::SUB_QUERY or right->type() == ExprType::SUB_QUERY;
        // exists / not exists 可能会使得 left_expr 为空
        if (left != nullptr && left->type() == ExprType::SUB_QUERY) {
        ...
        if (right != nullptr && right->type() == ExprType::SUB_QUERY) {
        ...
        if (not null_expr_exist and not is_sub_query and left->value_type() != right->value_type()) {
          auto left_to_right_cost = implicit_cast_cost(left->value_type(), right->value_type());
          auto right_to_left_cost = implicit_cast_cost(right->value_type(), left->value_type());
          if (left_to_right_cost <= right_to_left_cost && left_to_right_cost != INT32_MAX) {
            ExprType left_type = left->type();
          ...
          cmp_expr = (unique_ptr<ComparisonExpr>) new ComparisonExpr(static_cast<ComparisonExpr *>(condition.release())->comp(), std::move(left), std::move(right));
        }
        if (not cmp_expr)
        {
          cmp_expr = unique_ptr<ComparisonExpr>(static_cast<ComparisonExpr *>(condition.release()));
