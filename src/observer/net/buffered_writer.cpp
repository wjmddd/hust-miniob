...
RC BufferedWriter::clear()
{
  ...
  RC rc = RC::SUCCESS;
  while (OB_SUCC(rc) && buffer_.size() > 0) {
    rc = clear_internal(buffer_.size());
  }
  return rc;
}
...
RC BufferedWriter::clear_internal(int32_t size)
{
  if (fd_ < 0) {
    return RC::INVALID_ARGUMENT;
  }
  ...
  int32_t clear_size = 0;
  while (OB_SUCC(rc) && buffer_.size() > 0 && size > clear_size) {
    const char *buf = nullptr;
    int32_t read_size = 0;
    rc = buffer_.buffer(buf, read_size);
    if (OB_FAIL(rc)) {
    ...
    // 不进行文件写入，只是清空缓冲区
    clear_size += read_size;
    buffer_.forward(read_size); // 移动缓冲区的指针，清除数据
  }
